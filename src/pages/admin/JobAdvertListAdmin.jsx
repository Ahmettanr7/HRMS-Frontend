  ...
  let remove = (jobAdvertId) => {
    ...
    {jobAdverts.map((jobAdvert, key) => (
      <Table.Row key={key}>
        ...
        <Table.Cell>{jobAdvert.jobTypePlace.placeTypeName} - {jobAdvert.jobTypeTime.timeTypeName}</Table.Cell>
        <Table.Cell>{jobAdvert.advertDate}</Table.Cell>
        <Table.Cell>{jobAdvert.dueDate}</Table.Cell>
        <Table.Cell>
            {jobAdvert.activityStatus?
            <Button style={{width:'100%'}} color='green' active icon labelPosition="left"><Icon name='info'/>{jobAdvert.adStatusDescription}</Button>:
            <Button style={{width:'100%'}} color='yellow' active icon labelPosition="left"><Icon name='info'/>{jobAdvert.adStatusDescription}</Button>}
            </Table.Cell>
        <Table.Cell><Button  onClick={() => confirm(jobAdvert.jobAdvertId)} color='green' icon labelPosition="left"><Icon  name='checkmark' size='large'/> YAYINLA</Button></Table.Cell>
        <Table.Cell><Button  onClick={() => remove(jobAdvert.jobAdvertId)} color='red' icon labelPosition="left"><Icon name='remove circle' size='large' /> KALDIR</Button></Table.Cell>
      </Table.Row>
      ))}
    </Table.Body>
  </Table>
